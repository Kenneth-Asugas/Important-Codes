...
def open_role_form(role):
    ...
    formWindow = tk.Tk()
    formWindow.title(f"{role} Form - LearnSync")
    formWindow.state('zoomed')

    # Create main container
    main_container = tk.Frame(formWindow)
    main_container.pack(fill=tk.BOTH, expand=True)

    # Left panel for navigation and instructions
    left_panel = tk.Frame(main_container, width=250, bg="#9edcff")
    left_panel.pack(side=tk.LEFT, fill=tk.Y)
    left_panel.pack_propagate(False)  # Prevent the frame from shrinking

    # Header
    header_label = tk.Label(left_panel, text=f"{role} FORM", font=("Roboto", 14, "bold"),
                            bg="#9edcff", fg="black", pady=10)
    header_label.pack(fill=tk.X, pady=10)

    # Navigation buttons
    login_btn = tk.Button(left_panel, text="Log In", font=("Orbitron", 12),
                          bg="#FFC19E", fg="black", width=15, relief=tk.RAISED, pady=5,
                          command=lambda: show_login_form(content_frame, role))
    login_btn.pack(fill=tk.X,pady=10)

    signup_btn = tk.Button(left_panel, text="Sign up", font=("Orbitron", 12),
                           bg="#FFC19E", fg="black", width=15, relief=tk.RAISED, pady=5,
                           command=lambda: show_signup_form(content_frame, role))
    signup_btn.pack(fill=tk.X,pady=10)

    # Back button with arrow
    back_frame = tk.Frame(left_panel, bg="#9edcff")
    back_frame.pack(pady=10)

    back_arrow = tk.Label(back_frame, text="←", font=("Orbitron", 14), bg="#9edcff")
    back_arrow.pack(side=tk.LEFT)

    back_btn = tk.Button(back_frame, text="Back", font=("Orbitron", 12),
                         bg="#9edcff", fg="black", bd=0,
                         command=lambda: back_to_main(formWindow))
    back_btn.pack(side=tk.LEFT)

    # Instructions section
    instructions_label = tk.Label(left_panel, text="INSTRUCTIONS", font=("Orbitron", 12, "bold"),
                                  bg="#9edcff", fg="black", pady=10)
...
def create_background(parent):
    ...
    try:
        bg_image = Image.open(os.path.join(image_path, "BLURED LOGO COENGG.png"))
...
def show_login_form(content_frame, role):
    ...
    # Login form overlay
    login_frame = tk.Frame(content_frame, bg="#9edcff", bd=1, relief=tk.SOLID)
    login_frame.place(relx=0.5, rely=0.5, anchor=tk.CENTER, width=350, height=350)
    ...
    role_label = tk.Label(login_frame, text=f"{role} LOGIN", font=("Orbitron", 13, "bold"),
                          bg="#9edcff", fg="black")
    ...
    # Login button
    login_button = tk.Button(login_frame, text="Log in", bg="#FFC19E", fg="black",
                             font=("Orbitron", 12), width=12, bd=1,
                             command=lambda: handle_login(id_entry.get(), password_entry.get(), role))
    # Bind Enter key to trigger login
    def enter_key(event):
        handle_login(id_entry.get(), password_entry.get(), role)
    login_frame.bind_all('<Return>', enter_key)
...
def show_signup_form(content_frame, role):
    ...
    # Signup form overlay
    signup_frame = tk.Frame(content_frame, bg="#9edcff", bd=1, relief=tk.SOLID)
    signup_frame.place(relx=0.5, rely=0.5, anchor=tk.CENTER, width=350, height=470)

    # Create form header with logo
    create_form_header(signup_frame)

    # Role label for signup
    role_label = tk.Label(signup_frame, text=f"{role} SIGN UP", font=("Orbitron", 13, "bold"),
                          bg="#9edcff", fg="black")
    ...
    conf_password_frame = tk.Frame(signup_frame, bg="#9edcff")
    ...
    conf_password_entry.config(fg="gray", show="")
    ...
    eye_conf_btn = tk.Label(conf_password_frame, font=("Open Sans", 12), text="👁", bg="#9edcff", cursor="hand2")
    ...
    # Sign up button
    signup_button = tk.Button(signup_frame, text="Sign up", bg="#FFC19E", fg="black",
                              font=("Orbitron", 12), width=12, bd=1,
...
def create_password_field(parent):
    ...
    password_frame = tk.Frame(parent, bg="#9edcff")
    ...
    eye_btn = tk.Label(password_frame, font=("Orbitron", 12),text="👁", bg="#9edcff", cursor="hand2")
...
def create_form_header(parent):
    ...
    try:
        logo = load_image("LS.png", (120, 90))  # Adjust size if necessary
        container = tk.Frame(parent, bg="#9edcff")
        container.pack(pady=5)

        # Logo at the top
        logo_label = tk.Label(container, image=logo, bg="#9edcff")
        logo_label.image = logo  # Keep a reference
        logo_label.pack(pady=(0, 5))  # Small space between logo and text

        # "Learn Sync" text under the logo
        sync_label = tk.Label(container, text="Learn Sync", font=("Orbitron", 18, "bold"),
                              bg="#9edcff", fg="black")
        sync_label.pack()
        ...
        header_frame = tk.Frame(parent, bg="#9edcff")
        header_frame.pack(fill=tk.X, pady=5)

        sync_label = tk.Label(header_frame, text="Learn Sync", font=("Orbitron", 14, "bold"),
                              bg="#9edcff", fg="black")
        sync_label.pack()
...
def handle_login(id_num, password, role):
    ...
    try:
        ...
        # Check credentials
        cursor.execute(f"SELECT id, full_name FROM {table} WHERE id = ? AND password = ?", (id_num, password))
        ...
        if user:
            ...
            print("Launching dashboard at:", dashboard_path)  # Optional debug
            subprocess.Popen([sys.executable, dashboard_path, role, id_num])
        else:
...
def initialize_main_window():
    ...
    root = tk.Tk()
    root.title("LearnSync - LMS")
    root.state('zoomed')  # Make the window full screen
    root.configure(bg="#9edcff")

    # Create a frame to center the images
    image_frame = tk.Frame(root, bg="#9edcff")
    image_frame.pack(pady=60)

    # Load and display images
    photos = [
        (load_image(filename="UCC.png", size=(140, 130)),20),
        (load_image(filename="COE.png", size=(160, 170)),10),
        (load_image(filename="LS.png", size=(200, 215)),10)
        ]

    for photo, padx in photos:
        if photo:
            imageLabel = tk.Label(image_frame, image=photo, bg="#9edcff")
            imageLabel.image = photo
            imageLabel.pack(side=tk.LEFT, padx=padx)

    # University and College Name
    label1 = tk.Label(root, text="UNIVERSITY OF CALOOCAN CITY", font=("Orbitron", 45, "bold"), bg="#9edcff")
    label1.pack()
    label2 = tk.Label(root, text="COLLEGE OF ENGINEERING", font=("Orbitron", 20), bg="#9edcff")
    label2.pack(pady=20)

    # Buttons for Student and Instructor
    frame = tk.Frame(root, bg="#9edcff")
    frame.pack(pady=20)

    buttons = [
        tk.Button(frame, text="STUDENT", font=("Orbitron", 14, "bold"), bg="#FFC19E", fg="black", width=20, height=2, command= open_student_form),
        tk.Button(frame, text="INSTRUCTOR", font=("Orbitron", 14, "bold"), bg="#FFC19E", fg="black", width=20, height=2, command= open_instructor_form)
        ]
...
image_path = os.path.normpath(base_path)
